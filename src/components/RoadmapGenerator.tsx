...
export const RoadmapGenerator = ({ jobData, onBack, onGenerateTodoList }: RoadmapGeneratorProps) => {
  ...
  const [aiGeneratedRoadmap, setAiGeneratedRoadmap] = useState<any[]>([]);
  const [isGeneratingAi, setIsGeneratingAi] = useState(false);
  
  const { generatePersonalizedRoadmap, loading: aiLoading, error: aiError } = useRoadmapAi();
    ...
    try {
      ...
      if (userSkills.length > 0) {
        try {
          ...
          console.log('Job market data fetched successfully:', { insights, jobsCount: jobs.length });
        } catch (error) {
          console.error('Error fetching job market data:', error);
          // Provide fallback job market data
          jobMarketData = {
            insights: {
              totalJobs: 1000,
              topCompanies: ['Google', 'Microsoft', 'Amazon', 'Apple', 'Meta'],
              averageSalary: '$80,000 - $120,000',
              inDemandSkills: userSkills.slice(0, 5),
              jobTrends: ['Remote work increasing', 'AI/ML skills in demand', 'Full-stack development popular']
            },
            jobs: []
          };
        }
      ...
      const roadmapParams = {
        jobData: {
          job: jobData.job,
          description: jobData.description || `A ${jobData.job} position`,
        ...
        userProfile: {
          ...
          careerGoals: knowledgeProfile?.career_goals || [],
          graduationDate: graduationDate.toISOString().split('T')[0],
          freeTimePerWeek: freeTimeSchedule.reduce((total, slot) => total + (slot.hours || 0), 0) || 10
        },
        jobMarketData
      };

      // Generate AI roadmap
      const aiRoadmap = await generatePersonalizedRoadmap(roadmapParams);
      setAiGeneratedRoadmap(aiRoadmap);
    } catch (error) {
  ...
  const roadmapItems = aiGeneratedRoadmap.length > 0 ? aiGeneratedRoadmap : generateRoadmapItems();

  const handleGenerateRoadmap = async () => {
    setShowRoadmap(true);
    // Generate AI roadmap when showing the roadmap
    if (aiGeneratedRoadmap.length === 0) {
      ...
      <div className="space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-2xl font-bold text-gray-900">AI-Generated Learning Roadmap</h2>
              <p className="text-sm text-gray-600">
                {aiGeneratedRoadmap.length > 0 
                  ? "Personalized roadmap based on your profile and current job market"
          ...
          <div className="flex gap-2">
            ...
            >
              ...
              {isGeneratingAi || aiLoading ? 'Generating...' : 'Regenerate'}
            </Button>
            ...
            <CardContent className="p-6">
              <div className="flex items-center justify-center gap-3">
                ...
                <div className="text-center">
                  <h3 className="font-semibold text-gray-900">Generating Your Personalized Roadmap</h3>
                  <p className="text-sm text-gray-600">Analyzing your profile, job requirements, and current market trends...</p>
                </div>
              </div>
            </CardContent>
          </Card>
        ...
        {aiError && (
          <Card className="border-red-200 bg-red-50">
            <CardContent className="p-6">
              <div className="text-center">
                <h3 className="font-semibold text-red-900">Error Generating AI Roadmap</h3>
        ...
        {/* Roadmap Items */}
        {!isGeneratingAi && !aiLoading && !aiError && (
          <>
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        <CardContent className="p-6">
          <Button onClick={handleGenerateRoadmap} className="w-full gap-2" size="lg" disabled={isGeneratingAi || aiLoading}>
            {(isGeneratingAi || aiLoading) ? (
              <>
                <Loader2 className="h-4 w-4 animate-spin" />
