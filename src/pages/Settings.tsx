  ...
  const loadProfile = async () => {
    try {
      ...
      if (profileData) {
        ...
        setTargetJobTitle(profileData.target_job_title || '');
        setLocation(profileData.location || '');
        setTimeCommitment(profileData.time_commitment || 'moderate');
        setPreferredLearningStyle(profileData.preferred_learning_style || 'mixed');
      }
    } catch (error: any) {
      toast({
    ...
    <SidebarProvider>
      ...
      <Dialog open={showGitHubIntegration} onOpenChange={setShowGitHubIntegration}>
        <DialogContent className="max-w-4xl max-h-[80vh] overflow-y-auto">
          ...
          <GitHubIntegration onSyncComplete={handleGitHubSyncComplete} />
        </DialogContent>
      </Dialog>
    </SidebarProvider>
  );
